...
public class HelloApplication extends Application {
    ...
    private Node[] bigBang() {
        ...
        for (int i = 0; i < planets.length; i++) {
            ...
            planets[i].setName(names[i]);
            planets[i].setSize(planets[i], sizes[i]);
            planets[i].setProperties(planets[i], xProperties[i]);
           //planets[i].setRotation(planets[i], rotationSpeed[i]);
            planets[i].setCoat(planets[i], coatsLocation[i]);
        }
    ...
    private void animation() {
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long l) {
                ...
                /*planet.setRotationAxis(new Point3D(0,1,0));//fori cycle for all the planets
                planet.rotateProperty().set(planet.getRotate()+rotationSpeed);*/
            }
        };timer.start();
    }
    ...
    public void start(Stage stage) throws IOException {
        ...
        //universe.getChildren().add(new AmbientLight(Color.WHITESMOKE.darker()));

        camera.setNearClip(0.01);
        camera.setFarClip(5000);
        //camera.setTranslateZ(-100);
        /*camera.getTransforms().addAll(new Translate(90,0,300),
                new Rotate(11,Rotate.Y_AXIS));*/

        Scene scene = new Scene(root, stage.getMaxHeight(), stage.getMaxWidth(), true);
    ...
    private Node[] saturnRings() {
        /*Cylinder ring = new Cylinder(160, 10);
        PhongMaterial m = new PhongMaterial();
        m.setDiffuseMap(new Image("C:/Users/sabča/Desktop/programovani/zkouska/src/main/resources/saturnRing.png"));
        ring.setMaterial(m);
        ring.getTransforms().add(new Translate(planets[6].getTranslateX(),planets[6].getTranslateY(),0));*/
    ...
    private Node background() {
        ...
        //Image image = new Image("C:/Users/sabča/Desktop/programovani/zkouska/src/main/resources/background_2.jpg"); <- 2k
    ...
    private void initMouseControl(Group group, Scene scene, Stage stage){//detects every mouse movement
        ...
        xRotate.angleProperty().bind(newX);
        yRotate.angleProperty().bind(newY);*/
        ...
        scene.setOnMouseDragged(mouseEvent -> {
            ...
            /*group.setTranslateX(mouseEvent.getSceneX()-startingX);
            group.setTranslateY(mouseEvent.getSceneY()- startingY);*/
            /*newX.set(new Translate(x+mouseEvent.getSceneX(),0,0));
            newY.set(y + mouseEvent.getSceneX());*/// + protože v javě y dolů je +
        });
    ...
    private void planetInteractions(Sphere[] spheres) {
        ...
        /*PhongMaterial m =new PhongMaterial(Color.AQUA);

        for (int i = 0; i < 9; i++) {
            int finalI = i;
            spheres[i].setOnMouseEntered(mouseEvent -> {
                circle(spheres[finalI], universe, true);
            });
            spheres[i].setOnMouseExited(mouseEvent -> {
                circle(spheres[finalI], universe, false);
            });
        }
        spheres[0].setOnMouseEntered(mouseEvent -> {
            spheres[0].setMaterial(m);
        });*/

    }

    private void action(Sphere sphere) {
        AnimationTimer timer = new AnimationTimer() {
            @Override
            public void handle(long l) {
