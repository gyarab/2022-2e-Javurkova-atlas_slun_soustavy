...
public class Main extends Application {
    ...
    /**
     * Metoda start je první metoda která se při puštění kódu spustí.
     *
     * Do již deklarované proměnné Group universe přidám veškeré potřebné objekty pro zobrazení toho co chci v programu vidět.
     *
     * Poté deklaruji proměnnou Group root do které přidám proměnnou universe a objekty vrácené metodou buttons(),
     * toto udělám protože proměnná universe je kontrolována myšší - upřesněno u metody planetInteractions().
     *
     * Upřesním si atributy u již deklarované proměnné camera a přidám ji ke scéně scene.
     *
     * Vytvořím a deklaruju proměnou Image im kterou využiji jako kurzor aplikace.
     *
    ...
    public void start(Stage stage) throws IOException {
        ...
        universe.getChildren().addAll(planetTitles());
     ...
     *
     * Metoda obsahuje pouze jeden for cyklus ve kterém se po jednom vytvoří 9 potřebných objektů Planet.
     *
     * Vrací pole planets, což je pole 9ti vytvořených Planet.
     */
    ...
    /**
     * Metoda moon() vrací Sphere (sféru) moon.
     *
     * Jako první vytvořím proměnnou PhongMaterial m, které přiřadím DiffuseMap a poté "obleču" sféru moon.
     *
    ...
    /**
     * Metoda sunlight() vrací Pointlight pl, což zobrazuje "svit Slunce".
     *
    ...
    /**
     * Metoda planetTitles() je velmi podobná metodě planetHighlights().
     *
     * Vrací pole planetsTitles[] typu Text, které slouží jako jména zobrazené pod planetou.
     */

    private Node[] planetTitles() {
    ...
    /**
     * Metoda saturnRings() vrací pole saturnRings typu Circle, jenž zobrazuje prstence saturnu.
     *
     ...
     *
     * To udělám vytvořením proměnné StringBuilder info do kterého pomocí cyklu while a try-catch vložím text z textového souboru.
     * Vytvořím proměnnou  Label systemInfo ve které zobrazím uložený text a upřesním u něj detaily.
     *
     *  Poté vytvořím a definuji proměnnou Rectangle systemInfoBox, což je box, ve kterém se text bude zobrazovat.
     *
     *  Do objektu StackPane popUp přídám obě proměnné text i systemInfoBox, abych s nimi mohla naráz manipulovat.
     *
     *  Vytvořím pole buttons typu Button do jehož uložím dvě proměnné Button, jedna s textem "Info", druhá s "Exit".
     *
    ...
    /**
     *  Metoda animation() je nevracející metoda a stará se o rotaci všech proměnných typu Planet z pole planets a rotaci měsíce.
    ...
    /**
     * Metoda planetInteractions() se stará o animaci zmíněných proměnných z pole planetsHighlights[] a planetTitles[].
     *
     ...
     *
     *  Metoda setOnMouseClicked() volá metodu action() s atributem finalI, což je index planety na kterou bylo kliknuto.
     */
    ...
    /**
     * První metoda která není volaná počateční metodou start je metoda action() s atributem int index.
     ...
     *
     * Prvně přidám již definované proměnné TranslateTransition zoomInX (zaslouží se o posunutí planety po souřadnici X při přibližování) objekt který bude animovat a X souřadnici do které objekt půjde.
     * To samé udělám pro proměnou TranslateTransition zoomOutX, akorát ten bude objekt přesouvat na jeho výchozí polohu, tedy na již definovanou proměnnou z pole xProperties.
     *
     * K již definované proměnné TimeLine zoomInSize přidám nový KeyFrame, který bude mít na starosti zvětšení poloměru určené planety na 220.
     * To samé udělám u definované proměnné TimeLine zoomOutSize, akorát zde se poloměr planety bude vracet na svoji původní velikost.
     *
     * U podmínky if(!zoomedIn) zjistím zda je planeta již příblížena nebo ne.
     * Zda není přiblížena zavolám metodu prepareInfoBox() s atributem int index, která připraví informační okno o planetě.
     * Poté zavolám metodu planetsInfoBox() s atributy boolean zoomIn - dá metodě vědět zda je planeta přiblížena nebo ne - a s atributem StackPane currentInfoBox.
     *
     *  Proměnnou zoomedIn definuju jako true. Pouštím animace zoomInX a zoomInSize.
     *  Jestli je zvolenou planetou Slunce -tedy proměnná z pole planets[] s indexem 0- pouštím již definovanou animaci zoomInPlSun, jestli je to jakákoli jiná planeta pouštím animaci zoomInPl.
     *  Tyto animace se starají o pohyb proměnné Pointlight pl -tedy "svit Slunce".
     *
     *  Poté pomocí for cyklu a animaci TimeLine tlIn zmenším poloměry všech planet na které nebylo kliknuto.
     *  Pomocí cyklu foreach udělám to samé i pro proměnné z pole saturnRings[].
     *
     *
     *  V případě že je již na planetu přiblíženo spouštím animaci zoomOutSize a zoomOutX. Jestli je zvolenou planetou Slunce pouštím animaci zoomOutPl.
     ...
     *
     *  Za pomocí proměnné Stringbuilder planetInfo a BufferedReader reader načtu informace o zvolené planetě z jejího textového souboru - ten určím za pomocí int index.
     *
     *  Vytvořím si proměnnou Label text do které vložím text z proměnné planetInfo.
     *  Poté proměnné text dodám detaily. Vytvořím si proměnnou Rectangle box a StackPane infoBox do které vložím již definované proměnné text a box.
     */
    ...
    /**
     * Metoda planetsInfoBox() se stará o to aby se již připravený StackPane infoBox přiližoval, nebo zmenšoval - to závisí na atributu boolean zoom.
     *
     * Nejprve si vytvořím proměnnou ScaleTransition scaleIn, které přiřadím souřadnice, časovou hodnotu a objekt.
     * Poté udělám to samé pro proměnnou ScaleTransition scaleOut.
     *
    ...
    /**
     *
     * Metoda initMouseControl() s atributy Scene scene a Stage stage se stará interakci "vesmíru" s kurzorem.
     *
     * Prvně vytvořím proměnné Translate tX, tY, které následně přiřadím k proměnné universe.
     * Souřadnici X tX spojím s již definovanou proměnnou DoubleProperty newX.
     * To samé udělám pro souřadnici Y tY a DoubleProperty newY.
     *
     * Metoda setOnMousePressed() reaguje na interakci myši se scénou scene.
     ...
     *
     *  Pomocí metody addEventHandler() s atributem ScrollEvent.SCROLL přibližuju a oddaluju proměnnou universe.
     *  Do proměnné double movement uložím pohyb kolečka na myši, jenž pak odečtu od souřadnice Z proměnné universe.
     */
